...
def test_create_menu():
    response = client.post(
        "/api/v1/menus",
        json={"title": "Menu 1", "description": "Description 1", "submenus_count": 0, "dishes_count": 0},
    )
    assert response.json() == {
        "id": "1",
        "title": "Menu 1",
...
def test_patch_menu():
    response = client.patch("/api/v1/menus/1", json={"title": "update", "description": "update"})
    assert response.json() == {
        "id": "1",
        "title": "update",
        "description": "update",
...
def test_patch_not_found_menu():
    response = client.patch("/api/v1/menus/0", json={"title": "update", "description": "updated menu"})
    assert response.json() == {"detail": "menu not found"}
...
def test_submenu_post():
    response = client.post(
        ...
        json={"title": "My submenu 1", "description": "My submenu description 1", "dishes_count": 0},
    )
    assert response.json() == {
        "id": "1",
        "title": "My submenu 1",
...
def test_post_dish():
    ...
    assert response.json() == {"id": "1", "title": "dish 1", "description": "dish description 1", "price": "5.5"}


def test_patch_dish():
    response = client.patch(
        "/api/v1/menus/2/submenus/2/dishes/1",
        json={"title": "update dish", "description": "update dish description", "price": "10.5"},
    )
    assert response.json() == {
        "id": "1",
        "title": "update dish",
